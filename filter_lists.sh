...
# Output files
ads_tracker_output_sophos="/var/www/html/ads_tracker_sophos.txt"
ads_tracker_output_adguard="/var/git/Block-Lists/ads_tracker_adguard.txt"
amp_output_sophos="/var/www/html/amp_sophos.txt"
...
native_tracker_output_sophos="/var/www/html/native_tracker_sophos.txt"
...
grep -vwFf "$whitelist_ads_tracking_sophos" "$temp_ads_tracker_sophos" | sort -u > "$ads_tracker_output_sophos"
grep -vf "$whitelist_ads_tracking_adguard" "$temp_ads_tracker_adguard" | sort -u > "$ads_tracker_output_adguard"

# Remove whitelisted entries from the DoH blocklist
grep -vwFf "$whitelist_doh_sophos" "$temp_doh_sophos" | sort -u > "$doh_output_sophos"
...
grep -vFf "$whitelist_ads_tracking_sophos" "$temp_native_sophos" | sort -u > "$native_tracker_output_sophos"
